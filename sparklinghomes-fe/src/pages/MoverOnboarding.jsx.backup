...
import { FiUpload, FiMapPin, FiClock, FiTruck, FiShield, FiFileText, FiCreditCard, FiEye, FiDownload, FiX } from 'react-icons/fi';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext.jsx';
import { moversService } from '../services/movers.js';

const MoverOnboarding = () => {
  ...
  const serviceMapping = {
    ...
    'Furniture Assembly': 'furniture-moving',
  ...
  // Redirect if not authenticated or not a mover
  useEffect(() => {
    if (!authLoading) {
      if (!isAuthenticated) {
        navigate('/login');
        ...
        // If mover is already approved, redirect to dashboard
        navigate('/mover/dashboard');
      } else if (user && user.verificationDocuments && user.verificationDocuments.length > 0) {
        // If mover has uploaded documents, they've completed onboarding
        navigate('/mover/dashboard');
      }
    }
  ...
  useEffect(() => {
    if (user && userType === 'mover') {
      // Map verification documents to form document structure (matching MoverProfile)
  ...
  const handleFileUpload = async (file, documentType) => {
    ...
    // Map form document keys to backend document types (matching MoverProfile)
    const documentTypeMapping = {
      'businessLicense': 'license',
      'proofOfInsurance': 'insurance',
      'vehicleRegistration': 'vehicle',
      'backgroundCheck': 'background', // Match MoverProfile expectation
    ...
    try {
      // Use the onboarding movers service for document upload
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess('Mover profile created successfully! You are now verified. Redirecting to subscription...');
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {['Residential Moving', 'Commercial Moving', 'Long Distance', 'Local Moving', 'Packing', 'Storage', 'Furniture Assembly', 'Piano Moving'].map((service) => (
  ...
  const renderStep4 = () => (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-600">
        Upload the following legally required documents for your moving business. 
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Become a Mover</h1>
...
export default MoverOnboarding;
